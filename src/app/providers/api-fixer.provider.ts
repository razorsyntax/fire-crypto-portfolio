...
/**
 * We switched from different API's. This was used as a dictionary to standardize.
 * A refactor could eliminate this from the front-end. A back-end should serve the data.
 */
...
export class ApiFixerProvider {
  ...
  getCorrectCryptoSymbol(givenSymbol) {
    return this.symbolFix.coins[givenSymbol]
      ? this.symbolFix.coins[givenSymbol].sym
      : givenSymbol;
  }

  getApiSymbol(givenSymbol) {
    return this.symbolFix.coins[givenSymbol]
      ? this.symbolFix.coins[givenSymbol].apisymbol
      : givenSymbol;
  }

  getActualName(givenName) {
    return this.symbolFix.coins[givenName]
      ? this.symbolFix.coins[givenName].name
      : givenName;
  }

  getActualExchangeName(givenName) {
    return this.exchangeNames[givenName]
      ? this.exchangeNames[givenName].name
      : givenName;
  }

  getExchangeId(givenExchangeID) {
    const exchangeObj: string[] = Object.keys(this.exchangeNames).filter(
      item => this.exchangeNames[item].name === givenExchangeID
    );
