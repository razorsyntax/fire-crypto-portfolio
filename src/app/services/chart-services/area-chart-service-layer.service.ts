...
export class AreaChartServiceLayerService {
  ...
  candleSubscriptionArray(docs) {
    ...
    if (docs.length > 0) {
      ...
      const tempDatesArr = docs.map(x => this.dayCalculation(dateTimeMillis, x.data.transactions));
      const ohlcvCallArr = [];
      // aseembling custom OHLCV Call per coin
      // LIMIT CALLS TO 99
      for (let i = 0; i < tempDatesArr.length; i++) {
        const oldestDate: number = tempDatesArr[i];
        const dateFromDaysAgo: number = moment().subtract(oldestDate + 1, 'd').unix();
        const currentTime: number = moment().unix();
        const sym: string = this.portfolioSymbols[i];

        if (this.coinIdsProvider.hasCGid(sym)) {
          ohlcvCallArr.push(this.getDataService.getMarketChart(this.coinIdsProvider.coin_ids[sym].id_cg, dateFromDaysAgo, currentTime, 'USD'));
        }
      }
      return ohlcvCallArr;
    } else {
  ...
  getHistoricalData(forkJoinSubScriptionArray: Observable<{}[]>[]): Observable<{}[]> {
  ...
  oldestTransaction(coinTransactions: Transactions[]): number {
    ...
    if (coinTransactions.length > 1) {
      return Math.min.apply(Math, coinTransactions.map(o => o.datetime));
    }
  }
  ...
  calculateTotals(priceList: [number, number][]) {
    ...
    for (let i = 0; i < Object.keys(totalsObj).length; i++) {
      ...
      totalsObj[symbol] = this.combineTotalsObj(priceList[i], symbol, totalsObj);
    }
    ...
    if (finalTotals.length > 0) {
      ...
      const formattedValues = combinedValues.map(item => this.combinedItems(item));
  ...
  sampleData(data, sampleCount: number) {
    if (data.length >= sampleCount) {
      return this.sampler.largestTriangleThreeBuckets(data, sampleCount, 'Amount', 'Date');
    }
    return data;
  }
  ...
  combinedDates(megaDates) {
    const combinedValues = _.values(_.groupBy(megaDates, ([d]) => moment(d).startOf('day')));
    let tempArr = [];
    let finalValues = [];
  ...
  getDailyDates(datelist) {
    ...
    const days = _.values(_.groupBy(datelist, ([d]) => moment(d).startOf('day')));
  ...
  combineTotalsObj(priceList, symbol, totalsObj): { total: number, date: string }[] {
    ...
    if (isOldestDate) {
      const additionalDates = this.createOldestDailies(dailyDates, oldestDateISO);
      const megaDates = additionalDates.concat(dailyDates);
      dailyDates = this.combinedDates(megaDates);
    }

    for (let i = 0; i < dailyDates.length; i++) { // 1 Day
      const dailyPrice = dailyDates[i];
      const priceDate: string = moment(dailyPrice[0]).format('DD/MM/YYYY');

      let todaysTransactions = transactions.filter(transaction => moment(transaction.datetime).format('DD/MM/YYYY') === priceDate);

      if (todaysTransactions === undefined) {
        todaysTransactions = [];
      }

      holdingsAmount = (todaysTransactions.length > 0) ? (this.utilities.calculateTotalAmount(todaysTransactions) + holdingsAmount) : holdingsAmount;

      total = holdingsAmount * dailyPrice[1];

      newTotals.push({ total: total, date: moment(dailyDates[i][0]).format('MM/DD/YYYY') });
    }
    return newTotals;
  };

  createOldestDailies(dailyDates, isoldestDate) {
    const dailies = this.datetimeUtilities.getAllDatesBetweenTwoDatesFormatted(isoldestDate, moment().valueOf());
